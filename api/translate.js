...
module.exports = async (req, res) => {
  ...
  if (!text || typeof text !== 'string' || text.trim().length > 1000) return res.status(400).json({ error: 'Provide text up to 1,000 characters.' });
  if (!ALLOWED_LANGUAGES.has(selectedLanguage)) return res.status(400).json({ error: 'Choose Japanese, Mandarin, or Cantonese.' });
  if (!process.env.OPENAI_API_KEY) return res.status(503).json({ error: 'Translation service has not been configured yet.' });

  const schema = { type: 'object', additionalProperties: false, properties: {
    translation: { type: 'string' }, source_language: { type: 'string' }, target_language: { type: 'string' }, direction: { type: 'string', enum: ['from_english', 'to_english'] },
  ...
  try {
    const openaiResponse = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        model: process.env.OPENAI_TRANSLATION_MODEL || 'gpt-5.5', reasoning: { effort: 'low' },
        ...
        input: text.trim(), text: { format: { type: 'json_schema', name: 'travel_translation', strict: true, schema } },
      }),
    });
    if (!openaiResponse.ok) {
      const upstream = await openaiResponse.json().catch(() => ({}));
      const message = upstream?.error?.message || 'Translation service could not complete the request.';
      console.error('OpenAI translation request failed', openaiResponse.status, upstream?.error?.type || 'unknown_error');
    ...
    console.error('OpenAI translation response could not be processed', error.message);
